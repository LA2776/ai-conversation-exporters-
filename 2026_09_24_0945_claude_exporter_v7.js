// =============================================================
// Claude.ai Conversation Exporter v7 — Scroll-and-Collect
// Handles virtualized DOM (long conversations where middle
// messages are unloaded from the DOM as you scroll).
//
// v7 changes: Updated selectors for Sept 2026 DOM
//   - User messages: .cds-user-message-body (was [class*="font-user"])
//   - Claude messages: .standard-markdown, [class*="font-claude"]
//   - Turn containers: [class*="group/message-row"]
//
...
(async () => {
  ...
  // Selectors — updated Sept 2026
  const USER_SEL = '.cds-user-message-body, [class*="font-user"]'; // new primary + old fallback
  const CLAUDE_SEL = '.standard-markdown, [class*="font-claude"]';
  const TURN_ROW_SEL = '[class*="group/message-row"]'; // new turn-level wrapper
  ...
  function extractText(el) {
    ...
    clone.querySelectorAll(
      ...
      '[class*="timestamp"], [class*="tooltip"], .sr-only'
    ).forEach(n => n.remove());
  ...
  // Store collected turns by fingerprint to avoid duplicates
  const collectedTurns = new Map(); // fingerprint → { role, text, order }
  let turnPositionCounter = 0;

  const collectCurrentTurns = () => {
    // Strategy A: use group/message-row containers directly
    let containers = document.querySelectorAll(TURN_ROW_SEL);

    // Strategy B: find common ancestor of user + claude elements
    if (containers.length === 0) {
      ...
      containers = commonParent.children;
    }
    ...
    Array.from(containers).forEach(child => {
      ...
      if (hasUser && !hasClaude) role = 'user';
        ...
        // Check if this looks like user text (short, no markdown formatting)
        // or if the element itself has user-message classes
        const cls = child.className?.toString() || '';
        const innerCls = child.innerHTML || '';
        if (cls.includes('user') || innerCls.includes('cds-user') || innerCls.includes('font-user')) {
          role = 'user';
        } else {
          role = 'unknown';
        }
